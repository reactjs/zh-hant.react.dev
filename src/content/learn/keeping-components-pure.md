...
有些 JavaScript 的函數為「純函數」。純函數只會執行計算，不會做別的事情。如果我們嚴格地把 Component 都寫成純函數，就可以在隨著代碼庫的增長中避免一系列令人困惑且不可預期的問題出現。但是在獲得這些好處前，你必須先遵守一些規則。
...
在上面的範例中， `double` 是一個**純函數**。如果你傳入 `3`，他永遠都會回傳 `6`。
